...
indir_non_data_moments = "./data/model_inputs/large_model_scrap_and_price_from_eqb/"
...
X_dep = utils.create_data_dependent_regressors(
    tab_index=tab_index,
    prices=prices,
...
# Doing some manual corrections to the data to make it consistent with MLE approach
## First for scrap correction term
scrap_correction_cols = X.filter(like="scrap_correction").columns
X["scrap_correction"] = X[scrap_correction_cols].sum(axis=1)
X = X.drop(columns=scrap_correction_cols)
model_specification = [
    col for col in model_specification if col not in scrap_correction_cols
] + ["scrap_correction"]
breakpoint()
# Second for psych transaction cost
psych_trans_cols = X.filter(like="buying_").columns
X["buying"] = X[psych_trans_cols].sum(axis=1)
X = X.drop(columns=psych_trans_cols)
model_specification = [
    col for col in model_specification if col not in psych_trans_cols
] + ["buying"]
...
for i in range(8):
    ax = axs[i // 4, i % 4]
    ax.plot(ev_dummies_est_full[i, :], label="ddr", marker="+")
    ax.plot(ev_dummies_model[i, :], label="matlab")
...
def name_placeholder(df, col_matches, col_nmatches):
    ...
    df["variablename"] = df["variablename"].replace(
        "car_type_[0-9]+_[0-9]+.*", "u_0", regex=True
    )
    df["variablename"] = df["variablename"].replace(
        "car_type_[0-9]+_x.*", "u_a", regex=True
    )
...
est["variablename"] = est.reset_index()[
    "level_0"
].values  # Reverses the ordering of the variable string
est["matches"] = est["variablename"].str.findall("([0-9]+)")
...
def extract_coefficients_from_struct(struct, varname):
    ...
    ).rename(columns={0: "Coefficient_matlab"})
