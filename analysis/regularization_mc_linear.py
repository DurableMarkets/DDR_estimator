...
import logit.estimators.optimal_wls3 as optimal_wls3
...
sim_options, mc_options, params_update, options_update, specification, out_dir=get_model_specs(
    lambda model_name: f"./output/simulations/{model_name}/regwls/"
    )
...
for eps in tqdm(epss, desc="Monte Carlo studies"):
    ...
    for i in tqdm(chunks, desc="Current MC progress", leave=False):
        ...
        # Estimate 
        est, est_post = optimal_wls3.ridge_regression_mc(
            ccps=cfps, 
            counts=counts, 
            X=X, 
        ...
        est["Nbar"] = int(Nbar)
        est['eps'] = eps
        est = est.set_index(['eps', "Nbar", "mc_iter"], append=True)
...
# Renaming indexes
runs.index.names = ["coefficients",'eps', "Nbars", "mc_iter"]
...
p_975 = runs.groupby(["coefficients", "eps"], sort=False).quantile(0.975)
