...
used_car_prices[:,-1] = prices['scrap_car_prices']
prices['used_car_prices'] = used_car_prices.flatten()
...
def name_placeholder(df, col_matches, col_nmatches):
    ...
    df['variablename']=df['variablename'].replace('car_type_([0-9]+|all)_([0-9]+|all).*', 'u_0', regex=True)
    df['variablename']=df['variablename'].replace('car_type_([0-9]+|all)_x.*', 'u_a', regex=True)
    df['variablename']=df['variablename'].replace('ev_dums_.*', 'EV term', regex=True)
    df['variablename']=df['variablename'].replace('scrap_correction.*', 'sigma_s', regex=True)
    df['variablename']=df['variablename'].replace('buying.*', 'psych_transcost', regex=True)


    df=df[['consumer_type', 'car_type', 'variablename', 'Coefficient']]
    df=df.set_index(['variablename', 'consumer_type', 'car_type',])
...
def extract_coefficients_from_struct(struct, varname):
    ...
    # Setting new index
    coeffs = coeffs.set_index(['variablename', 'consumer_type','car_type',]).rename(columns={0: 'Coefficient_matlab'})
...
est_matlab=pd.concat([u_0, u_a, mum, sigma_s,psych_transcost,], axis=0)
# join  
...
plt.close('all')
plt.plot(prices_data_eqb['used_car_prices'], label='eqb')
plt.plot(prices['used_car_prices'],label='dep. rate = '+str(disc_fac))
plt.legend()
plt.savefig(out_dir + "used car prices comparison.png", dpi=300, bbox_inches="tight")
